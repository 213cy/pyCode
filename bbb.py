...
class MyHTTPRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def copyfile_by_range(self, fin, fout, start, end):
        ...
        if end < 0:  # 代表原始Range请求未指定完整范围，只指定了开始位置
            # FIXME：健壮性fix，如果读到内容小于size参数？需要判断len(buf)
    ...
    def do_GET(self):
        ...
        # 原来的send_head这个函数实现有点莫名其妙？
        f, range = self.send_head()
        if f:
            if range:
    ...
    def list_directory(self, path):
        ...
        def compare_by_modtime(x, y):
            ...
            if stat_x.st_mtime < stat_y.st_mtime:
                ...
                return 0
        # 最近修改的排在前面
        list.sort(lambda x, y: compare_by_modtime(y, x))
    ...
    def send_head(self):
        ...
        if m:  # TODO: 重构这里的代码
            ...
            self.end_headers()
            # 注意，这个地方不需要urllib.quote
            self.wfile.write('<video src="%s" controls></video>' % video_path)
            return (None, None)
                ...
                if m:
                    ...
                    if end < 0:
                        content_range_header_value = "bytes %d-%d/%d" % (
                            start, filesize-1, filesize)
                    else:
                        content_range_header_value = "bytes %d-%d/%d" % (
                            start, end, filesize)
                    self.send_header(
                        "Content-Range", content_range_header_value)
